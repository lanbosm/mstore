...
vueFingerConstructor.prototype = {
  ...
  move: function (e) {
    ...
    if ((currentX && Math.abs(e.distanceX) > 30) || (currentY && Math.abs(e.distanceY) > 30)) {
      e.direction = this._swipeDirection(this.x1, this.x2, this.y1, this.y2)
      if (!this.__fLock) {
        this.__fLock = this._swipeAxis(e.direction);
      }
      e.swipeAxis = this.__fLock
  ...
  _swipeAxis: function (d) {
    return d =='Left' || d== 'Right'? 'X' : 'Y'
  }
}
